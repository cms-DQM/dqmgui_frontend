...
var Browser = function Browser() {
  ...
  var lumisectionsChangeHandler = function lumisectionsChangeHandler(lumi) {
    ...
    Object(_containers_display_utils__WEBPACK_IMPORTED_MODULE_13__["changeRouter"])(Object(_containers_display_utils__WEBPACK_IMPORTED_MODULE_13__["getChangedQueryParams"])({
      lumi: lumi
    }, query)); //setLumisection from store(using useContext) set lumisection value globaly.
    //This setted value is reachable for lumisection browser in free search dialog (you can see it, when search button next to browsers is clicked).
    ...
    //FREE SEARCH DIALOG: lumisection browser value in free search dialog is changed it HASN'T to be setted to url immediatly, just when button 'ok' 
    //in dialog is clicked THEN value is setted to url. So, useContext let us to change lumi value globaly without changing url, when wee no need that.
    //And in this handler lumi value setted to useContext store is used as initial lumi value in free search dialog. 
...
//# sourceMappingURL=index.js.3e4385a32e9b421ccf8e.hot-update.js.map