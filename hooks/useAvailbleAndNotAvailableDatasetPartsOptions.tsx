...
export const useAvailbleAndNotAvailableDatasetPartsOptions = (
  ...
  const selectedDatasetParts = getDatasetNameSplitBySlashIntoObject(currentDataset);
  
  const datasetPartsPositions = Object.keys(selectedDatasetParts).sort()
  const { results_grouped } = useSearch(
    run_number,
    ''
  );
  ...
  const [lastSelectedDatasetPartPosition, setLastSelectedDatasetPartPosition] = useState(firstPosition);

  //lastSelectedDatasetPartOption: is VALUE of last selected dataset part
  const [lastSelectedDatasetPartValue, setLastSelectedDatasetPartValue] = useState(selectedDatasetParts[firstPosition]);
  ...
  const [selectedParts, setSelectedParts] = useState<DatasetPartsProps>(getDatasetNameSplitBySlashIntoObject(currentDataset));
  ...
  const partsObjectArray = getDatasetParts(allDatasets, lastSelectedDatasetPartPosition);
  ...
  const doesCombinationOfSelectedDatasetPartsExists = allDatasets.includes(fullDatasetName);
